...
class BaseDatos(object):
	...
	def storeData(self, values, sensor_code, board_code):
			...
			daily_reg = self.createFile(sensor_code)
	...
	def createFile(self,sensor_code):
			
			# Directorio donde se va a crear el archivo
			# path = "/home/leo/Documentos/TESIS/logs"+'/'+sensor_code
			path = "/var/tmp"+'/'+sensor_code # Cambiado el 29-05
						...
						for fname in files:
							...
							if mtime > max_mtime:
								...
								if now_time == time_last_file:
									...
									# Archivo viejo debo crear uno nuevo
									daily_reg = open(path+'/'+date_reg+'.log','a+')
									return daily_reg
